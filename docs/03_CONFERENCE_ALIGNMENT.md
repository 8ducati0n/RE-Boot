...
### 4.1 Fernandes et al. (2025) — Performance & Metacognition Disconnect

> ❗ **v2 정정 노트 (2026-05-20)**: 본 절 v1은 *"EDM 2025 Best Paper — Tang & Bosch. Trust but Risk Over-reliance"*를 인용했으나 해당 논문은 *존재하지 않는 환각 인용*으로 확인됨. 실제 보유 PDF(`논문/Performance and Metacognition Disconnect when Reasoning in Human-AI Interaction.pdf`)는 아래 Fernandes et al. (2025) arXiv preprint이며, 핵심 주장이 *over-reliance*가 아닌 *performance-metacognition disconnect*임. 본 절은 정정된 출처에 맞춰 *재서술됨*.

- **논문**: Fernandes, D., Villa, S., Nicholls, S., Haavisto, O., Buschek, D., Schmidt, A., Kosch, T., Shen, C., & Welsch, R. (2025). *Performance and Metacognition Disconnect when Reasoning in Human-AI Interaction*. arXiv:2409.16708.
- **핵심 주장**: 사람이 AI와 함께 추론할 때 *수행은 향상*되지만 *메타인지(자기 수행 평가의 정확도)는 오히려 분리·왜곡*된다. 즉 "잘 했는지"를 *알아채는 능력*이 AI 사용 중에 손상된다.
- **RE:Boot 구현 위치**: `backend/apps/recommendations/models.py` — `AIRecommendation`의 **2-Tier Trust** 필드 (`auto_executable` / `requires_teacher_approval`) + 사전/사후 자기평가 격차 시각화(`web/src/app/checklist/page.tsx`)
- **학술 기여도**: Fernandes et al.이 *진단*한 *수행-메타인지 분리*를 **아키텍처 레벨로 대응** — (1) 낮은 리스크 액션만 자동 집행, 고위험은 교수자 게이트, (2) 사전/사후 격차를 학습자에게 직접 노출해 *분리 자체를 가시화*.
...
A. Fernandes et al. (2025)이 지적한 *수행-메타인지 분리* 문제는 두 가지 장치로 대응합니다. (1) **소크라테스식 제한 프롬프트** — 튜터는 정답을 직접 주지 않고 단계적 힌트를 제공합니다(`backend/apps/tutor/prompts/socratic_policy.py`). (2) **교수자 게이트** — 학습자에게 전달되기 전 고위험 제안은 반드시 인간을 거칩니다. 추가로 사전/사후 자기평가 격차 데이터로 메타인지 분리 신호를 추출해 교수자에게 경고를 보냅니다.
...
- Fernandes, D., Villa, S., Nicholls, S., Haavisto, O., Buschek, D., Schmidt, A., Kosch, T., Shen, C., & Welsch, R. (2025). Performance and metacognition disconnect when reasoning in human-AI interaction. *arXiv preprint arXiv:2409.16708*. (❗ v1의 "Tang, Y., & Bosch, N. (2025). Trust but risk over-reliance... EDM 2025 Best Paper" 표기는 환각 인용으로 확인되어 정정)
